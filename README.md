...
The findings below consolidate all evidence and observations recorded in the source material, organized by activity. Corresponding screenshots are provided in `/media` and linked in the Evidence column of each row.

### A. Individual OSINT Verification — Dr. Martha Ozohu Musa (Nwigwe University)

| Finding | Risk Level | Evidence |
|---|---|---|
| The Wayback Machine (archive.org) was then used and returned archived evidence confirming that Dr. Martha Ozohu Musa was previously a lecturer at Nwigwe University. | **Low** — *MITRE ATT&CK: T1589.003 — Gather Victim Identity Information: Employee Names (Reconnaissance)* | ![Fig. A1](media/image3.png)<br>*Fig. A1 — Wayback Machine archived capture confirming prior affiliation with Nwigwe University.* |

### B. niitph.com — Domain, IP & Contact Exposure

| Finding | Risk Level | Evidence |
|---|---|---|
| A CentralOps-based domain dossier query resolved niitph.com / www.niitph.com to the IP address 176.123.0.55, obtained using the OSINT Framework. | **High** — *MITRE ATT&CK: T1590.005 — Gather Victim Network Information: IP Addresses (Reconnaissance); T1596.002 — Search Open Technical Databases: WHOIS (Reconnaissance)* | ![Fig. B1](media/image4.png)<br>*Fig. B1 — niitph.com IP address, phone number links, and location address.* |
| The website publishes a direct contact widget: two phone numbers, an enquiries mailbox (niitportharcourtenquiries@gmail.com), and a physical address (No. 1 Kaduna Street, D-Line, Port Harcourt). | **High** — *MITRE ATT&CK: T1589.002 — Gather Victim Identity Information: Email Addresses (Reconnaissance); T1591.001 — Gather Victim Org Information: Determine Physical Locations (Reconnaissance)* | ![Fig. B2](media/image5.png)<br>*Fig. B2 — Contact details widget.* |

### C. niitph.com — Staff Identity & Email Exposure

| Finding | Risk Level | Evidence |
|---|---|---|
| Staff names and email links associated with niitph.com were identified using the OSINT Framework and Maltego, including staff member Ibigbari Eretoru, Chief Counsellor, whose work email address is published on a public profile. | **High** — *MITRE ATT&CK: T1589.002 — Email Addresses; T1589.003 — Employee Names (Reconnaissance); Enables T1566 — Phishing (Initial Access)* | ![Fig. C1](media/image6.png)<br>*Fig. C1 — Staff name and email exposure.* |

### D. niitph.com — Social Media Presence

| Finding | Risk Level | Evidence |
|---|---|---|
| Google Dorking identified an active @niitphc Instagram account promoting the institute's activities. | **High** — *MITRE ATT&CK: T15xx — Social Media (Reconnaissance)* | ![Fig. D1](media/image7.png)<br>*Fig. D1 — Social media presence (Instagram).* |

### E. niitph.com — Human Attack Surface

| Finding | Risk Level | Evidence |
|---|---|---|
| A staff member was identified as Centre Head, NIIT Port Harcourt Centre, on LinkedIn using the OSINT Framework. | **High** — *MITRE ATT&CK: T1589.003 — Employee Names; T1591.004 — Identify Roles (Reconnaissance)* | ![Fig. E1](media/image8.png)<br>*Fig. E1 — Staff identified on LinkedIn.* |
| A staff member's personal Facebook profile, publicly linked to the organization, was also identified using the OSINT Framework. | **High** — *MITRE ATT&CK: T1593.001 — Social Media; T1589.003 — Employee Names (Reconnaissance)* | ![Fig. E2](media/image9.png)<br>*Fig. E2 — Staff identified on Facebook.* |
| A direct review of the website's "Meet the Staff" page named faculty by role: Data Analysis Faculty, Multimedia Faculty, Java Faculty, and Accounts. | **High** — *MITRE ATT&CK: T1591.004 — Gather Victim Org Information: Identify Roles (Reconnaissance)* | ![Fig. E3](media/image10.png)<br>*Fig. E3 — Staff / faculty webpage.* |

### F. niitph.com — Infrastructure Accessibility

| Finding | Risk Level | Evidence |
|---|---|---|
| Nmap identified multiple open mail-related ports (110, 143, 465, 587, 993, 995) on niitph.com, indicating exposed POP3/IMAP/SMTP services that could be leveraged for credential brute-forcing or phishing infrastructure abuse. | **High** — *MITRE ATT&CK: T1595.002 — Active Scanning: Vulnerability Scanning (Reconnaissance) → enables T1110 Brute Force / T1566 Phishing* | ![Fig. F1](media/image11.png)<br>*Fig. F1 — Nmap host-discovery and port-scan output for niitph.com (176.123.0.55).* |
| The same scan found web ports 80, 443, and 8080 open on niitph.com, confirming publicly accessible web services that could be targeted for exploitation of the public-facing application. | **Medium** — *MITRE ATT&CK: T1595.001 — Scanning IP Blocks (Reconnaissance) → enables T1190 Exploit Public-Facing Application* | ![Fig. F2](media/image12.png)<br>*Fig. F2 — Nmap output highlighting web ports 80, 443, and 8080 open.* |
| Port 53 (DNS) was also found open on niitph.com, indicating the DNS service is reachable from the public internet and could be enumerated during reconnaissance. | **Medium** — *MITRE ATT&CK: T1590.002 — DNS; T1596.001 — DNS/Passive DNS (Reconnaissance)* | ![Fig. F3](media/image13.png)<br>*Fig. F3 — Nmap output highlighting DNS port 53 open.* |
| Critical administrative and database ports — SSH 22, SMTP 25, MySQL 3306, and PostgreSQL 5432 — were confirmed closed to public access on niitph.com, reducing the exposed attack surface for these services. | **Low** — *MITRE ATT&CK: No offensive technique applicable — reduces attack surface* | ![Fig. F4](media/image14.png)<br>*Fig. F4 — Nmap output highlighting ports 22, 25, 3306, 5432 closed.* |
...
- `README.md` — this document (full report content)
- `media/` — all figures/screenshots embedded in the original Word document (`image1.png`–`image14.png`), referenced above by figure number
